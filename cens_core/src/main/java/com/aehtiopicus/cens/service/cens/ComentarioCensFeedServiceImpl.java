...
public class ComentarioCensFeedServiceImpl implements ComentarioCensFeedService {
	...
	private static final String MATERIAL_SQL = "SELECT cp.nombre as pnombre, ca.nombre as canombre, cc.nombre as ccnombre, cc.yearcurso, "
			...
			+ ",cmm.nombre as cmmnombre, cmm.id as cmmid, cmm.nro as nro, cmm.fecha_cambio_estado "
			+ "FROM cens_material_didactico cmm  " + "INNER JOIN cens_programa cp ON cmm.programa_id = cp.id "
			+ "INNER JOIN cens_asignatura ca ON cp.asignatura_id = ca.id "
			+ "INNER JOIN cens_curso cc on cc.id = ca.curso_id " + "WHERE cmm.id = :id";
	...
	private static final String FEEDS_EMAIL = "SELECT ccf.fecha_creacion, ccf.id_dirigido, ccf.prefil_dirigido, ccf.comentariotype, ccf.notificado, ccf.id, cc.tipoId "
			...
			+ "INNER JOIN cens_comentario cc ON cc.id = ccf.comentariocensid " + "WHERE cu.username = :username "
			+ "AND ccf.notificado = false " + "AND ccf.visto = false ";
	...
	public List<NotificacionComentarioFeed> getGeneratedFeeds(String userName, boolean email) throws CensException {
		...
		try {
			List<Object[]> resultList = entityManager.createNativeQuery(email ? FEEDS_EMAIL : FEEDS_APP)
	...
	public Map<String, String> getCommentSource(NotificacionTypeComentarioIdKey ctik) throws CensException {
		try {
			...
			case TE_MATERIAL:
			case MATERIAL:
				resultMap = getMaterialResult(
						entityManager.createNativeQuery(MATERIAL_SQL).setParameter("id", ctik.getTipoId()),
						ctik.getComentarioType());
				break;
			case TE_PROGRAMA:
			case PROGRAMA:
				resultMap = getProgramaResult(
						entityManager.createNativeQuery(PROGRAMA_SQL).setParameter("id", ctik.getTipoId()),
						ctik.getComentarioType());
				break;
			case TE_ASIGNATURA:
				resultMap = getAsignaturaResult(
						entityManager.createNativeQuery(ASIGNATURA_SQL).setParameter("id", ctik.getTipoId()));
				break;
	...
	private Map<String, String> getAsignaturaResult(Query q) {
		Object[] result = (Object[]) q.getSingleResult();

		Map<String, String> resultMap = new HashMap<>();
		resultMap.put(CensServiceConstant.COMENTARIO_ASIGNATURA, result[0].toString());
		resultMap.put(CensServiceConstant.COMENTARIO_ASIGNATURA_ID, result[3].toString());

		resultMap.put(CensServiceConstant.COMENTARIO_CURSO, result[1].toString());
		resultMap.put(CensServiceConstant.COMENTARIO_CURSO_ID, result[4].toString());

		resultMap.put(CensServiceConstant.COMENTARIO_CURSO_YEAR, result[2].toString());

		resultMap.put(CensServiceConstant.ESTADO_REVISION, EstadoRevisionType.INEXISTENTE.name());

		resultMap.put(CensServiceConstant.FECHA_CAMBIO_ESTADO, result[5].toString());

		return resultMap;
	}

	private Map<String, String> getProgramaResult(Query q, ComentarioType comentarioType) {
		Object[] result = (Object[]) q.getSingleResult();
		...
		if (comentarioType.equals(ComentarioType.TE_PROGRAMA)) {
			resultMap.put(CensServiceConstant.FECHA_CAMBIO_ESTADO, result[8].toString());
		}

		return resultMap;
	}

	private Map<String, String> getMaterialResult(Query q, ComentarioType comentarioType) {
		Object[] result = (Object[]) q.getSingleResult();
		...
		if (comentarioType.equals(ComentarioType.TE_MATERIAL)) {
			resultMap.put(CensServiceConstant.FECHA_CAMBIO_ESTADO, result[11].toString());
		}
	...
	public void markAllFeedsForUserAsNotified(String username) throws CensException {
		try {
			entityManager
					.createNativeQuery(
							"UPDATE cens_comentario_feed   SET notificado = true, ultima_notificacion = :ultima_notificacion "
									...
									+ "AND id_dirigido <> id_creador and ultima_notificacion is null ")
					.setParameter("username", username).setParameter("ultima_notificacion", new java.util.Date())
					.executeUpdate();
		} catch (Exception e) {
			logger.error("Error ", e);
	...
	public List<Long> getAsesoresIdExcludingCaller(Long fromId) {
		...
		List<Object> objList = entityManager
				.createNativeQuery(
						"SELECT cmc.id FROM cens_miembros_cens as cmc INNER JOIN cens_usuarios as cu ON cmc.usuario_id = cu.id "
								+ "INNER JOIN cens_perfil_usuario_cens as cpuc  ON cpuc.usuario_id = cu.id "
								+ "WHERE cmc.id <> :fromId AND cpuc.perfiltype = 'ROLE_ASESOR' ")
				.setParameter("fromId", fromId).getResultList();
	...
	@Override
	@Transactional
	public void markAllFeedsFromCommentsAsRead(Long id, List<ComentarioCens> comentarioList) throws CensException {
		try {
			StringBuilder sb = getAllCommentsIds(comentarioList);
			String result = sb.toString().substring(0, sb.length() - 1);
			entityManager
					.createNativeQuery(
							"UPDATE cens_comentario_feed SET visto = true WHERE id_dirigido = :idMiembroCens "
									+ "AND comentariocensid in (" + result + ")")
					.setParameter("idMiembroCens", id).executeUpdate();
		} catch (Exception e) {
	...
	public List<NotificacionComentarioFeed> getUnReadFeeds() throws CensException {
		...
		try {
			List<Object[]> resultList = entityManager.createNativeQuery(
					"SELECT ccf.fecha_creacion, ccf.id_dirigido, ccf.prefil_dirigido, ccf.comentariotype, ccf.notificado, ccf.id, cc.tipoId,trim(replace((replace(concat((CURRENT_DATE - (ultima_notificacion + INTERVAL '"
							+ days + " days'))),'days','')),'day','')) , ccf.ultima_notificacion "
							+ "FROM cens_comentario_feed as ccf  INNER JOIN  "
							+ "cens_miembros_cens as  cmc ON (cmc.id = ccf.id_dirigido AND cmc.id <> ccf.id_creador) "
							+ "INNER JOIN cens_usuarios as cu ON cu.id = cmc.usuario_id  "
							...
							+ "AND ccf.ignorado = false " + "AND ccf.ultima_notificacion + INTERVAL " + "'" + days
							+ " days' < CURRENT_DATE")
					.getResultList();
			if (CollectionUtils.isNotEmpty(resultList)) {
				NotificacionComentarioFeed ncf = null;
	...
	public int markCommentsAsIgnored(Long tipoId, ComentarioType tipoType) throws CensException {
		try {
			return entityManager
					.createNativeQuery("update cens_comentario_feed SET ignorado = true "
							+ "WHERE comentariocensid in (select cc.id from cens_comentario as cc "
							+ "WHERE cc.tipoid = :tipoid AND cc.tipocomentario = :tipotype)")
					.setParameter("tipoid", tipoId).setParameter("tipotype", tipoType.name()).executeUpdate();
		} catch (Exception e) {
