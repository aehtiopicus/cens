...
import com.aehtiopicus.cens.dto.cens.GeneralNotificacionDto;
		...
		if (notificacionesForUser != null) {
			...
			if (notificacionesForUser.getData() != null) {
				...
				if (notificacionesForUser.getData().containsKey(NotificacionType.COMENTARIO.name())) {
					cnDtoSet = convertCursoList(
							(Map) notificacionesForUser.getData().get(NotificacionType.COMENTARIO.name()));

					if (CollectionUtils.isNotEmpty(cnDtoSet)) {
						GeneralNotificacionDto cnDto = new GeneralNotificacionDto();
				...
				if (notificacionesForUser.getData().containsKey(NotificacionType.ACTIVIDAD.name())) {
					cnDtoSet = convertCursoList(
							(Map) notificacionesForUser.getData().get(NotificacionType.ACTIVIDAD.name()));

					if (CollectionUtils.isNotEmpty(cnDtoSet)) {
						GeneralNotificacionDto anDto = new GeneralNotificacionDto();
						anDto.setCurso(cnDtoSet);
						nDto.setActividad(anDto);
					}
				}

				if (notificacionesForUser.getData().containsKey(NotificacionType.TIEMPO_EDICION.name())) {
					cnDtoSet = convertCursoList(
			...
			nDto.calculateCantidadNotificaciones();
		}
		return nDto;
	}

	private Set<CursoNotificacionDto> convertCursoList(Map<ComentarioType, List<Map<String, String>>> mapData) {
		// comentarios
		...
		if (mapData != null && !mapData.isEmpty()) {
			...
			for (Map.Entry<ComentarioType, List<Map<String, String>>> comentarioListDataMap : mapData.entrySet()) {
				...
				for (Map<String, String> comentarioData : comentarioListDataMap.getValue()) {
					// for(Map.Entry<String, String> comentarioData :
					// comentarioListData.entrySet() ){
					cnDto = new CursoNotificacionDto();
					cnDto.setId(Long.valueOf(comentarioData.get(CensServiceConstant.COMENTARIO_CURSO_ID)));
					cnDto.setNombre(comentarioData.get(CensServiceConstant.COMENTARIO_CURSO) + ", ("
							+ comentarioData.get(CensServiceConstant.COMENTARIO_CURSO_YEAR) + ")");
					...
					if (comentarioListDataMap.getKey().equals(ComentarioType.PROGRAMA) || 
							comentarioListDataMap.getKey().equals(ComentarioType.MATERIAL) || 
							comentarioListDataMap.getKey().equals(ComentarioType.TE_PROGRAMA)|| 
							comentarioListDataMap.getKey().equals(ComentarioType.TE_MATERIAL)) {
						...
						if (!aDto.getPrograma().contains(pDto)) {
							...
							for (ProgramaNotificacionDto aux : aDto.getPrograma()) {
								if (aux.equals(pDto)) {
									if (CollectionUtils.isEmpty(aux.getMaterial())
											&& comentarioData.containsKey(CensServiceConstant.COMENTARIO_MATERIAL_ID)) {
										aDto.getPrograma().add(pDto);
										break;
									}
									if (!comentarioListDataMap.getKey().equals(ComentarioType.MATERIAL)) {
										aux.setCantidadComnetarios(aux.getCantidadComnetarios() + Integer
												.parseInt(comentarioData.get(CensServiceConstant.COMENTARIO_CANTIDAD)));
									}
						...
						if (comentarioListDataMap.getKey().equals(ComentarioType.MATERIAL) || comentarioListDataMap.getKey().equals(ComentarioType.TE_MATERIAL)) {
							...
							mDto.setNro(Integer.parseInt(
									comentarioData.get(CensServiceConstant.COMENTARIO_MATERIAL_NRO).toString()));
							...
							if (!pDto.getMaterial().contains(mDto)) {
								...
								for (MaterialNotificacionDto aux : pDto.getMaterial()) {
									if (aux.equals(mDto)) {
										aux.setCantidadComnetarios(
												aux.getCantidadComnetarios() + mDto.getCantidadComnetarios());
										break;
									}
								}
							}
						...
						setSpecificNotificationData(aDto, comentarioData, false);
					}
				}
				// }
			}
