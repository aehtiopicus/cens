...
public class UsuarioCensValidator {
	...
	public void validate(Usuarios usuario, boolean usernameOnly)
	...
	public void validateChangeUsername(Usuarios u, UsuariosDto uDto) throws Exception{
		if ( u == null || uDto == null) {
			throw new CensException("No hay datos de usuario");
		}
	...
	public void validateChangePass(Usuarios u, PasswordChangeDto passChangeDto)
			throws CensException {
		if (u == null) {
		...
		if (passChangeDto == null) {
			throw new CensException(
					"No existe informaci&oacute;n de cambio de contrase&ntilde;a");
		}
		Map<String, String> errorMap = new HashMap<String, String>();
		if (StringUtils.isEmpty(passChangeDto.getOldPass())) {
			errorMap.put("passwordOld",
					"Contrase&ntilde;a actual inv&aacute;lida");
		} else {
			if (!u.getPassword().equals(passChangeDto.getOldPass())) {
				...
				if (StringUtils.isEmpty(passChangeDto.getNewPass())) {
					errorMap.put("password",
							"Contrase&ntilde;a nueva inv&aacute;lida");
				}
				if (StringUtils.isEmpty(passChangeDto.getOldPass())) {
					errorMap.put("passwordNewConfirm",
							"Contrase&ntilde;a nueva inv&aacute;lida");
				}
				if (StringUtils.isNotEmpty(passChangeDto.getNewPass())
						&& StringUtils.isNotEmpty(passChangeDto
								.getNewPassConfirm())) {
					if (!passChangeDto.getNewPass().equals(
							passChangeDto.getNewPassConfirm())) {
						errorMap.put("password",
								"Contrase&ntilde;a no coincide");
					} else {
	...
	public void validateChangeImage(MultipartFile file) throws CensException {
		try{
			MediaType.parseMediaType(file.getContentType());
		}catch(InvalidMediaTypeException e){
			throw new CensException("El tipo de archivo no es soportado");
		}
