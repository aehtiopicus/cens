	...
	<form:form method="POST" commandName="parametroDto">
		...
		<fieldset>
			...
			<div class="tituloForm">
				<c:choose>
					<c:when test="${parametroDto.entityId == null}">
						<h3>Alta de Prepaga</h3>					
					</c:when>
					<c:otherwise>
						<h3>Edici&oacute;n de Prepaga</h3>
					</c:otherwise>
				</c:choose>
			</div>
			...
			<div class="footerForm">
				...
				<button class="button" type="button" onclick="window.location='<%=request.getContextPath() %>/prepagas'">Cancelar</button>
			</div>
		</fieldset>
